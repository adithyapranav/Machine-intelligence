...
class KMeansClustering:
    ...
    def e_step(self, data):
        ...
        """
        #TODO
        dist_matrix=np.zeros((data.shape[0],self.n_cluster))
        i=int(0)
        while(i<self.n_cluster):
                    dist_matrix[:,i]=np.sqrt(np.sum(np.square(data-self.centroids[i,:]), axis=1))
                    i+=1
    ...
    def evaluate(self, data, cluster_assign):
        ...
        """
        #TODO
        error_value=np.zeros((data.shape[0]))
        i=int(0)
        while(i <self.n_cluster):
            error_value[cluster_assign==i]=np.sqrt(np.sum(np.square(data[cluster_assign==i]-self.centroids[i]), axis=1))
            i+=1       
        sse=np.sum(np.square(error_value))
        return sse
        pass