...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        """
        # TODO
        operator1 = self.history[1]
        operator2 = self.history[2]
        operator1.grad=np.zeros_like(operator1.arr)
        operator2.grad=np.zeros_like(operator2.arr)
        
        if operator1.requires_grad: 
                  operator1.grad += np.ones_like(operator1.arr)
                  
        if operator2.requires_grad: 
                  operator2.grad += np.ones_like(operator2.arr)
                  
        if gradients is None:
            return (operator1.grad,operator2.grad)
            
        if operator1.requires_grad: 
                  operator1.grad = np.multiply(np.ones_like(operator1.arr), gradients)
                  
        if operator2.requires_grad: 
                  operator2.grad = np.multiply(np.ones_like(operator2.arr), gradients)
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients is None:
        	if operator1.requires_grad:
        		operator1.grad =operator1.grad + np.matmul(np.ones_like(operator1.arr), operator2.arr.transpose())
        		
        	if operator2.requires_grad:
        		operator2.grad =operator2.grad + (np.matmul(np.ones_like(operator2.arr), operator1.arr)).transpose()
        		
        else:
        	if operator1.requires_grad:
        		operator1.grad =operator1.grad + np.multiply(np.matmul(np.ones_like(operator1.arr), operator2.arr.transpose()), gradients)
        		
        	if operator2.requires_grad:
        		operator2.grad =operator2.grad + np.multiply(np.matmul(np.ones_like(operator2.arr), operator1.arr).transpose(), gradients)
    ...
    def backward(self, gradients=None):
        ...
        if self.history[0] == 'add':
            ...
            gradd = self.grad_add(gradients)
            if self.history[2]:
                self.history[2].backward(gradd[1])
                
            if self.history[1]:
                self.history[1].backward(gradd[0])
                

            
        elif self.history[0] == 'matmul':
            gradd = self.grad_matmul(gradients)
            if self.history[1]:
                self.history[1].backward(gradd[0])
