...
def create_identity_numpy_array(order):
	...
	array=None
	array=np.identity(order,dtype=None)
	return array
...
def matrix_cofactor(array):
    ...
    m = np.shape(array)[0]
    Cofactor_A = np.zeros([m,m])
    for i in range(1,m+1):
        for j in range(1,m+1):
                subofA = np.delete(array,i-1,0) 
                subofA = np.delete(subofA,j-1,1)
                M_ij = np.linalg.det(subofA) 
                Cofactor_A[i-1,j-1] = pow(-1,i+j)*(np.around(M_ij,decimals=3))
    return Cofactor_A
...
def f1(X1,coef1,X2,coef2,seed1,seed2,seed3,shape1,shape2):
    ...
    W2=None
    B=None
    if((shape1[1]!=X1.shape[0])or(shape2[1]!=X2.shape[0])or(shape1[0]!=shape2[0])or(X1.shape[1]!=X2.shape[1])):
    	return -1
